...
public class CmdGiveaway extends DiscordCommand {
    ...
    public void create(SlashCommandEvent e, CommandEventData data) {
        ...
        String content = this.heart.gCnf().getNewGiveawayFormat().replace("{prize}", prize).replace("{winners}", String.valueOf(winners)).replace("{time}", "<t:" + endTimeMillis / 1000L + ":R>");
        ...
        message.addReaction(this.heart.gCnf().getGiveawayEmoji()).complete();
        ...
        this.heart.cnfMgr().save(this.heart.getConfig(GiveawayStorage.class));
